...
commit;
select * from payment;
update member set point = 4500 where user_id='mskim';
select * from member;

delete from payment;
select * from personalqna;
update personalqna set status = '답변완료' where bid = 17;
update personalqna set status = '답변완료', answer = '환불 절차는 결제시 환불동의서 내용에 따라 진행됩니다 해당 강좌는 수강시작일이 오래 남아있어 100% 환불 가능함을 알려드립니다. 감사합니다 ' where bid = 16;